  ...
  render() {
    ...
    const schemas = [
      ...
      [
        ...
        { name: "pos_chem_x", label: "Chimie X %", selector: { number: { min: 0, max: 100 } } },
        { name: "pos_chem_y", label: "Chimie Y %", selector: { number: { min: 0, max: 100 } } }
      ],
      // 3: Système (14)
      [
      ...
      [
        ...
        { name: "pos_cam_x", label: "X %", selector: { number: { min: 0, max: 100 } } },
        { name: "pos_cam_y", label: "Y %", selector: { number: { min: 0, max: 100 } } }
      ],
      // 5: Idéal
      [
    ...
    const tabs = ["Général", "Boutons", "Temp/Chim", "Système", "Caméra", "Idéal"];

    return html`
      <div class="tabs">${tabs.map((t, i) => html`<button class="${this._tab === i ? 'active' : ''}" @click=${() => this._selectTab(i)}>${t}</button>`)}</div>
      <div class="content"><ha-form .hass=${this.hass} .data=${this._config} .schema=${schemas[this._tab]} @value-changed=${this._valueChanged}></ha-form></div>
    `;
  }

  static styles = css`
    .tabs { display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 10px; border-bottom: 1px solid #444; padding-bottom: 8px; }
    button { background: #222; color: #fff; border: 1px solid #444; padding: 5px 8px; cursor: pointer; border-radius: 4px; font-size: 10px; }
    button.active { background: #00f9f9; color: #000; border-color: #00f9f9; font-weight: bold; }
  `;
}

/**
 * CARTE SPA AVEC CAMÉRA
 */
class SpaCard extends LitElement {
  ...
  render() {
    ...
    for (let i = 1; i <= 14; i++) {
      ...
      if (ent) {
        ...
        sysItems.push(html`<div class="sys-item"><ha-icon icon="${s.icon}" class="${s.active ? 'neon-text' : ''}"></ha-icon><span class="sys-label">${c[`sys_label_${i}`]}:</span><span>${s.val}${s.unit}</span></div>`);
      }
    }

    // Rendu Caméra
    const cameraBlock = c.camera_entity ? html`
      <div class="glass-box camera-box" style="left:${c.pos_cam_x || 5}%; top:${c.pos_cam_y || 70}%; width:${c.camera_width || 250}px;">
        <div class="box-title">DIRECT CAMÉRA</div>
        <div class="camera-container">
          <hui-image 
            .hass=${this.hass} 
            .cameraImage=${c.camera_entity} 
            cameraView="live"
          ></hui-image>
        </div>
      </div>
    ...
    return html`
      <ha-card style="background-image: url('${c.background_image || '/local/sparond2.jpg'}');">
        ...
        <div class="glass-box sys-box" style="left:${c.pos_elec_x || 5}%; top:${c.pos_elec_y || 38}%;">
        ...
        <div class="glass-box" style="left:${c.pos_ideal_x || 62}%; top:${c.pos_ideal_y || 38}%;">
  ...
  static styles = css`
    ha-card { height: 750px; background-size: cover; border: 3px solid #00f9f9; border-radius: 25px; position: relative; overflow: hidden; color: white; }
    ...
    /* Style Caméra */
    .camera-container { width: 100%; border-radius: 10px; overflow: hidden; border: 1px solid rgba(0,249,249,0.5); }
...
window.customCards.push({ type: "spa-card", name: "SPA Camera Glass", preview: true });
