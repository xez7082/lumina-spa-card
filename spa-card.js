...
/**
 * ÉDITEUR ULTRA-STABLE AVEC MÉMOIRE LOCALE
 */
  ...
  render() {
    ...
    const schemas = [
      ...
      [...Array.from({length: 14}, (_, i) => [
       ...
       { name: "pos_elec_x", label: "Position Système X (%)", selector: { number: { min: 0, max: 100, mode: "slider" } } },
       { name: "pos_elec_y", label: "Position Système Y (%)", selector: { number: { min: 0, max: 100, mode: "slider" } } }],

      // 4: CAMÉRA
      [{ name: "camera_entity", label: "Entité Caméra", selector: { entity: { domain: "camera" } } },
       { name: "camera_width", label: "Largeur (px)", selector: { number: { min: 100, max: 500 } } },
...
/**
 * CARTE SPA PRINCIPALE - HAUTEUR COMPACTE
 */
class SpaCard extends LitElement {
  ...
  _getState(entityId) {
    if (!this.hass || !entityId || !this.hass.states[entityId]) return { val: '?', unit: '', active: false, icon: 'mdi:circle-outline' };
  ...
  render() {
    ...
    // Rendu 14 entités Système
    const sysItems = [];
    for (let i = 1; i <= 14; i++) {
      const ent = c[`sys_entity_${i}`];
      if (ent) {
        const s = this._getState(ent);
        sysItems.push(html`<div class="sys-item"><ha-icon icon="${s.icon}" class="${s.active ? 'neon' : ''}"></ha-icon><span class="sys-lbl">${c[`sys_label_${i}`] || ''}:</span><span>${s.val}</span></div>`);
      }
    }
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      if (ent) {
        const s = this._getState(ent);
        topButtons.push(html`<div class="sw-btn ${s.active ? 'on' : ''}" @click=${() => this.hass.callService("homeassistant", "toggle", {entity_id: ent})}><div class="sw-lbl">${c[`switch_${i}_label`] || 'S'+i}</div><ha-icon icon="mdi:power"></ha-icon></div>`);
      }
    }
    ...
    return html`
      <ha-card style="background-image: url('${c.background_image || '/local/sparond2.jpg'}');">
        ...
        <div class="glass-box" style="left:${c.pos_temp_x || 5}%; top:${c.pos_temp_y || 15}%;">
        ...
        <div class="glass-box" style="left:${c.pos_chem_x || 5}%; top:${c.pos_chem_y || 25}%;">
        ...
        <div class="glass-box sys-box" style="left:${c.pos_elec_x || 5}%; top:${c.pos_elec_y || 37}%;">
          <div class="box-h">SYSTÈME</div>
          <div class="sys-grid">${sysItems}</div>
        </div>

        ${c.camera_entity ? html`
          <div class="glass-box" style="left:${c.pos_cam_x || 5}%; top:${c.pos_cam_y || 62}%; width:${c.camera_width || 220}px; padding: 5px;">
            <div class="box-h">CAMÉRA LIVE</div>
        ...
        ${c.show_ideal_table !== false ? html`
          <div class="glass-box" style="left:${c.pos_ideal_x || 65}%; top:${c.pos_ideal_y || 37}%;">
            <div class="box-h">CIBLES</div>
            <div class="id-l"><span>pH</span> <span>7.2-7.6</span></div>
            <div class="id-l"><span>Brome</span> <span>3.0-5.0</span></div>
            <div class="id-l"><span>TAC</span> <span>80-120</span></div>
          </div>
        ` : ''}
      </ha-card>
    `;
  ...
  static styles = css`
    ha-card { height: 650px; background-size: cover; background-position: center; border: 2px solid #00f9f9; border-radius: 20px; overflow: hidden; position: relative; color: white; }
    .main-title { position: absolute; top: 10px; left: 15px; font-weight: 900; color: #00f9f9; font-size: 11px; text-transform: uppercase; }
    .top-bar { position: absolute; top: 35px; left: 10px; right: 10px; display: grid; grid-template-columns: repeat(8, 1fr); gap: 4px; }
    .sw-btn { background: rgba(0,0,0,0.7); border: 1px solid #00f9f9; border-radius: 6px; padding: 4px; text-align: center; cursor: pointer; }
    .sw-btn.on { background: rgba(0,249,249,0.3); box-shadow: 0 0 8px #00f9f9; }
    .sw-lbl { font-size: 8px; font-weight: bold; margin-bottom: 1px; text-transform: uppercase; overflow: hidden; }
    .glass-box { position: absolute; background: rgba(0,0,0,0.6); backdrop-filter: blur(8px); border: 1px solid #00f9f9; border-radius: 12px; padding: 8px; min-width: 160px; }
    .box-h { color: #00f9f9; font-size: 9px; font-weight: bold; border-bottom: 1px solid rgba(0,249,249,0.2); margin-bottom: 4px; }
    .box-b { display: flex; align-items: center; gap: 6px; font-size: 11px; font-weight: bold; }
    .sys-box { min-width: 230px; }
    .sys-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2px 8px; }
    .sys-item { display: flex; align-items: center; font-size: 10px; font-weight: bold; }
    .sys-lbl { color: #bbb; margin: 0 4px; }
    .cam-frame { border-radius: 8px; overflow: hidden; }
    hui-image { width: 100%; height: auto; display: block; }
    .id-l { display: flex; justify-content: space-between; font-size: 10px; color: #00ff88; font-weight: bold; margin-bottom: 2px; }
    .neon { color: #00f9f9; filter: drop-shadow(0 0 3px #00f9f9); }
...
window.customCards.push({ type: "spa-card", name: "SPA FINAL 650", preview: true });
