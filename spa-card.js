...
class SpaCardEditor extends LitElement {
  ...
  render() {
    ...
    const schemas = [
      [{ name: "card_title", label: "Titre", selector: { text: {} } }, { name: "background_image", label: "Image fond (URL)", selector: { text: {} } }],
      [{ name: "btn_w", label: "Largeur %", selector: { number: { min: 10, max: 100 } } }, { name: "btn_h", label: "Hauteur px", selector: { number: { min: 20, max: 200 } } },
      ...
      [{ name: "entity_water_temp", label: "Eau", selector: { entity: {} } }, { name: "entity_ambient_temp", label: "Air", selector: { entity: {} } },
       { name: "pos_temp_x", label: "X%", selector: { number: { min: 0, max: 100, mode: "slider" } } }, { name: "pos_temp_y", label: "Y%", selector: { number: { min: 0, max: 100, mode: "slider" } } },
       ...
       { name: "chem_w", label: "Chimie Largeur px", selector: { number: { min: 100, max: 500 } } }, { name: "chem_h", label: "Chimie Hauteur px", selector: { number: { min: 50, max: 400 } } }],
      [...Array.from({length: 14}, (_, i) => [{ name: `sys_entity_${i+1}`, label: `Entité ${i+1}`, selector: { entity: {} } }, { name: `sys_label_${i+1}`, label: `Nom`, selector: { text: {} } }, { name: `sys_icon_${i+1}`, label: `Icone`, selector: { icon: {} } }]).flat(),
       { name: "pos_elec_x", label: "X%", selector: { number: { min: 0, max: 100, mode: "slider" } } }, { name: "pos_elec_y", label: "Y%", selector: { number: { min: 0, max: 100, mode: "slider" } } },
       { name: "sys_w", label: "Largeur px", selector: { number: { min: 100, max: 500 } } }, { name: "sys_h", label: "Hauteur px", selector: { number: { min: 50, max: 500 } } }],
      [{ name: "camera_entity", label: "Caméra", selector: { entity: { domain: "camera" } } }, { name: "camera_width", label: "Largeur px", selector: { number: { min: 100, max: 500 } } }, { name: "camera_height", label: "Hauteur px", selector: { number: { min: 100, max: 500 } } },
       { name: "pos_cam_x", label: "X%", selector: { number: { min: 0, max: 100, mode: "slider" } } }, { name: "pos_cam_y", label: "Y%", selector: { number: { min: 0, max: 100, mode: "slider" } } }],
...
class SpaCard extends LitElement {
  ...
  render() {
    ...
    return html`
      <ha-card style="background-image: url('${c.background_image || ''}');">
        ...
        <div class="gb" style="left:${c.pos_temp_x || 5}%; top:${c.pos_temp_y || 11}%; width:${c.temp_w || 150}px; height:${c.temp_h || 60}px;">
          <div class="bh">TEMPERATURES</div>
          <div class="bb"><ha-icon icon="mdi:thermometer"></ha-icon> ${this._getState(c.entity_water_temp).val}${this._getState(c.entity_water_temp).unit}</div>
          <div class="bb"><ha-icon icon="mdi:weather-windy"></ha-icon> ${this._getState(c.entity_ambient_temp).val}${this._getState(c.entity_ambient_temp).unit}</div>
        </div>

        <div class="gb" style="left:${c.pos_chem_x || 5}%; top:${c.pos_chem_y || 25}%; width:${c.chem_w || 200}px; height:${c.chem_h || 80}px;">
          <div class="bh">CHIMIE</div>
          <div class="chem-grid">
             <div class="bb"><ha-icon icon="mdi:ph"></ha-icon> ${this._getState(c.entity_ph).val}</div>
             <div class="bb"><ha-icon icon="mdi:bolt"></ha-icon> ${this._getState(c.entity_orp).val}${this._getState(c.entity_orp).unit}</div>
             <div class="bb"><ha-icon icon="mdi:opacity"></ha-icon> ${this._getState(c.entity_bromine).val}${this._getState(c.entity_bromine).unit}</div>
             <div class="bb"><ha-icon icon="mdi:test-tube"></ha-icon> ${this._getState(c.entity_alkalinity).val}${this._getState(c.entity_alkalinity).unit}</div>
          </div>
        </div>
        ...
        ${c.camera_entity ? html`<div class="gb" style="left:${c.pos_cam_x || 5}%; top:${c.pos_cam_y || 70}%; width:${c.camera_width || 180}px; height:${c.camera_height || 120}px; padding:2px; overflow:hidden;"><hui-image .hass=${this.hass} .cameraImage=${c.camera_entity} cameraView="live"></hui-image></div>` : ''}

        ${c.show_ideal_table !== false ? html`<div class="gb" style="left:${c.pos_ideal_x || 70}%; top:${c.pos_ideal_y || 42}%; width:${c.ideal_w || 140}px; height:${c.ideal_h || 100}px;">
          <div class="bh">CIBLES</div>
          <div class="id"><ha-icon icon="mdi:ph"></ha-icon> 7.2-7.6</div>
          <div class="id"><ha-icon icon="mdi:bolt"></ha-icon> 650-750</div>
          <div class="id"><ha-icon icon="mdi:opacity"></ha-icon> 3-5</div>
          <div class="id"><ha-icon icon="mdi:test-tube"></ha-icon> 80-120</div>
        </div>` : ''}
      </ha-card>
    `;
  }
  ...
  static styles = css`
    ...
    .sw { background: rgba(0,0,0,0.8); border: 1px solid #00f9f9; border-radius: 6px; padding: 2px; text-align: center; cursor: pointer; font-size: 8px; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; }
    ...
    .chem-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 4px; }
    .sys-g { display: grid; grid-template-columns: 1fr 1fr; gap: 2px 8px; overflow-y: auto; height: calc(100% - 20px); }
    .sys-i { display: flex; align-items: center; font-size: 10px; white-space: nowrap; }
    .id { display: flex; align-items: center; gap: 10px; color: #00ff88; font-size: 10px; font-weight: bold; margin-bottom: 2px; }
...
window.customCards.push({ type: "spa-card", name: "SPA FULL CUSTOM SIZE", preview: true });
